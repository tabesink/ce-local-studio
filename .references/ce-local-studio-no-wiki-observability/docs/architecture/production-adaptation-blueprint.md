...
Local Studio patterns were verified against `_references/local-studio/controller/src/app-context.ts`, `controller/src/http/app.ts`, `controller/src/http/security-middleware.ts`, `frontend/src/lib/api/create-api-client.ts`, `frontend/src/features/agent/runtime/`, `frontend/src/ui/`, `frontend/src/app/styles/globals/tokens.css`, `services/agent-runtime/src/server.ts`, and the root quality scripts. These are design references, not runtime dependencies.
