...
The baseline source snapshot is `_references/local-studio` commit `07be5be7ce69bea0c3118744ab90d148b010fce0`. A parity baseline must record that commit, the exact reference file, the Context Engine route/state, viewport, theme, screenshot path, masks, and any approved product/accessibility/security divergence. Moving `HEAD` is not a baseline update.
