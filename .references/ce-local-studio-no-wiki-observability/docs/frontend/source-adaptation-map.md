...
```text
_references/local-studio/frontend/src/
_references/local-studio-ce-v1/local-studio/frontend/src/
_references/local-studio-ce-v1/local-studio/specs/04-features/F-009-frontend-delivery/
```
...
1. Record source snapshot commit, file, license, and target file in `THIRD_PARTY_NOTICES`/provenance ledger.
...
- Runtime bundle contains no import/path/package dependency on `_references` or Local Studio packages.
