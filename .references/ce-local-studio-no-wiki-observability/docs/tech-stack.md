...
The Python manifest and lockfile are deleted in the dirty worktree but exist in Git `HEAD`; preserve the lockfile in a rebuild and pin final resolved versions rather than relying only on the minimum constraints above.
...
- Editable vendored runtime package at `vendor/lightrag`, promoted from read-only reference evidence under ADR-002.
...
```text
context_engine/
  app.py, config.py, db.py, models.py, security.py
  api/                 FastAPI routes, dependencies, error envelope
  services/            auth, runtime config, domain/source/index/chat/context/audit services
  tools/               runtime controller boundary
frontend/
  src/app/             App Router pages and composition roots
  src/features/        chat, documents, domains, graph, settings, navigation
  src/lib/api/         cookie-aware HTTP client, auth, SSE, error mapping
  src/components/      shell and reusable UI primitives
vendor/lightrag/       vendored retrieval runtime and proof contract
migrations/versions/   ordered Alembic history
specs/                 governance, product, architecture, contracts, features, quality, delivery
tests/                 backend contract/integration tests and OpenAPI snapshots
```
...
- CI includes a Docker integration workflow in Git `HEAD`.
- The reviewed CI uses Python 3.13 while the runtime image uses Python 3.12; rebuild CI should test the supported deployment version and optionally a compatibility version explicitly.
