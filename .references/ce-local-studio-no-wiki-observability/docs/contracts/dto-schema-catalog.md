...
UtcTimestamp    := RFC 3339 UTC string
...
RequestId       := opaque server value, 1..80
```
...
`EvidenceItemDto`, document metadata, and location anchors are exactly those in `document-and-evidence-contract.md`. A redacted turn has `assistantAnswer:null`, `evidence:[]`, and no accepted-ref labels.
...
## Capability error sets

| Capability | Additional stable codes |
| --- | --- |
| auth | `invalid_credentials`, `session_expired`, `csrf_invalid`, `account_unavailable` |
| domain | `domain_not_query_eligible`, `operation_conflict`, `stale_revision` |
...
| document/evidence | codes in `document-and-evidence-contract.md` |
