...
export const DEFAULT_RESUME_MAX_ATTEMPTS = 5;
...
export async function runResumableTurnStream(options: ReconnectOptions): Promise<void> {
  const maxAttempts = options.maxAttempts ?? DEFAULT_RESUME_MAX_ATTEMPTS;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    const boundedBackoff = Math.min(8_000, 250 * 2 ** (attempt - 1));
