...
The P1 trusted-ingress/session configuration contract uses fail-closed deployment settings: `CE_PUBLIC_ORIGIN` (one HTTPS origin outside explicit development mode), `CE_INTERNAL_HOSTS` (comma-separated private upstream Host allowlist), `CE_TRUSTED_BFF_PEERS` (comma-separated IP/CIDR allowlist), `CE_CSRF_SIGNING_KEY` (dedicated high-entropy key, distinct from credential encryption), and `CE_GRAPH_REF_KEY` (purpose-separated ≥32-byte key for opaque graph refs, distinct from CSRF and credential encryption; absent outside testing fails readiness closed). Session policy uses bounded positive `CE_SESSION_IDLE_TTL_SECONDS`, `CE_SESSION_TOUCH_INTERVAL_SECONDS`, `CE_LOGIN_THROTTLE_WINDOW_SECONDS`, `CE_LOGIN_THROTTLE_MAX_FAILURES`, and `CE_LOGIN_THROTTLE_BLOCK_SECONDS`. Absolute expiry remains `CE_SESSION_TTL_SECONDS`; idle TTL cannot exceed it and touch cadence must be strictly less than idle TTL. Production startup rejects missing, malformed, wildcard, insecure-origin, or contradictory values. Test-only defaults are allowed only when `testing=True` and never define deployment evidence.
