...
| P5-04 | DONE | P3-03,P5-03,P6-01,P10-03 | replace the placeholder runtime with one real private `app/vendor/lightrag` runtime per Knowledge Domain; keep PostgreSQL authoritative; inject immutable profile metadata through a sealed bootstrap; preserve schema-v2 handoff and index/retrieval ports; prove real submit→ready→marker-preserving retrieve→delete/absence, restart, and parallel cross-domain isolation; real provider embedding and complete parser→mapped-Evidence proof remain P10-05 — plan `docs/plans/2026-07-28-006-feat-p5-04-real-lightrag-runtime-plan.md`; evidence `docs/_scratch/p5-04-lightrag-real-runtime-evidence.md` |
...
The 2026-07-28 parser-to-retrieval pipeline review is dispositioned without restoring legacy architecture. P4-05 already closes normalized region persistence and authorized location/turn/viewer projection. P10-05 now owns the remaining production parser fidelity, bounded Reducto URL/asset handling, killable Docling execution, real immutable-profile embedding calls, realistic marker-boundary proof, and one complete source-bytes-to-mapped-Evidence staging path. P10-06 owns the separately contracted non-PDF governed-preview pipeline. P12-06 freezes the resulting parser/provider/renderer artifacts, while P12-07 consumes them through deployed browser, failure, capacity, privacy, and isolation scenarios. The read-only legacy implementation remains evidence for Docling item traversal, Reducto pointer resolution, asset materialization, and fixture shape only; its Redis locks, JSON domain manifests, provider metadata persistence, source-path heuristics, and competing `SourceChunk` authority remain rejected.
