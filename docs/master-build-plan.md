...
| P10 | Deployable application stack | IN_PROGRESS | P8-P9 | runnable Compose stack, explicit migrations/bootstrap, worker lifecycle, smoke path, and operator runbook pass — P10-01..P10-05 DONE; P10-06 governed preview generation open |
...
The 2026-07-28 parser-to-retrieval pipeline review is dispositioned without restoring legacy architecture. P4-05 already closes normalized region persistence and authorized location/turn/viewer projection. P10-05 closed production parser packaging, bounded Reducto URL/asset handling, killable Docling execution, sealed OpenAI embedding bindings, marker-boundary proof, and gated staging smoke (operator live digests remain for production-supported labels). P10-06 owns the separately contracted non-PDF governed-preview pipeline. P12-06 freezes the resulting parser/provider/renderer artifacts, while P12-07 consumes them through deployed browser, failure, capacity, privacy, and isolation scenarios. The read-only legacy implementation remains evidence for Docling item traversal, Reducto pointer resolution, asset materialization, and fixture shape only; its Redis locks, JSON domain manifests, provider metadata persistence, source-path heuristics, and competing `SourceChunk` authority remain rejected.
