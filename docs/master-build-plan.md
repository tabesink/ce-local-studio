...
| P5 | LightRAG indexing eligibility | DONE | P4 | per-domain vendored LightRAG topology, immutable-profile bootstrap, isolation, idempotent submit, readiness, retrieval provenance, delete/absence and recovery are proven — P5-01..P5-04 DONE; real provider packaging and parser-to-Evidence staging closed under P10-05 |
...
| P9 | Thin Next.js frontend | DONE | P1-P8 | login/chat/documents/settings and the then-approved reserved graph shell use only versioned APIs — P9-01..P9-07 DONE at that scope; the later user-directed enabled graph is a P12-07 contract/runtime/API/frontend vertical slice |
| P10 | Deployable application stack | DONE | P8-P9 | runnable Compose stack, explicit migrations/bootstrap, worker lifecycle, smoke path, operator runbook, MinIO, provider packaging, and governed preview generation — P10-01..P10-06 DONE |
...
| P12 | Production release and recovery | NOT_STARTED | P0-P11 | Phase 1 read-only Knowledge Domain graph, immutable artifacts, full admin Reducto→graph/index→chat→Evidence→PDF demonstration, deployed-path streaming, migration/rollback, security/load/backup/restore and runbooks pass |
...
| P5-04 | DONE | P3-03,P5-03,P6-01,P10-03 | replace the placeholder runtime with one real private `app/vendor/lightrag` runtime per Knowledge Domain; keep PostgreSQL authoritative; inject immutable profile metadata through a sealed bootstrap; preserve schema-v2 handoff and index/retrieval ports; prove real submit→ready→marker-preserving retrieve→delete/absence, restart, and parallel cross-domain isolation; real provider embedding and parser→mapped-Evidence staging closed under P10-05 — plan `docs/plans/2026-07-28-006-feat-p5-04-real-lightrag-runtime-plan.md`; evidence `docs/_scratch/p5-04-lightrag-real-runtime-evidence.md` |
...
`docs/_scratch/p10-05-provider-packaging-evidence.md`. Additive image gates
install `parsers`/`synthesis`/`embeddings` only when claimed; Docling runs behind
a killable process deadline; Reducto URL/asset resolve stays in private transport;
LightRAG shim binds sealed OpenAI embeddings (Bedrock/Ollama fail-closed;
...
| P9-03 | DONE | P4-P6 | documents/outline/preview and the then-approved graph-unavailable route through approved contracts — evidence `docs/_scratch/p9-03-documents-library-evidence.md`; the later Phase 1 graph enablement decision is owned end-to-end by P12-07 |
...
| P9-06 | DONE | P9-01..P9-05 | Option A full workstation HTML gallery — 39 primitive/shared/feature targets have manifest + script-free HTML + React parity trios; non-routable gallery index and hard catalog↔docs↔files gate pass; HTML steers look while React/Vitest owns behavior — evidence `docs/_scratch/p9-06-full-gallery-evidence.md`; plan `docs/plans/2026-07-28-002-feat-full-workstation-html-gallery-plan.md`; P12-07 retains route Playwright / Settings F3 |
| P9-07 | DONE | P9-02,P9-03,P9-04,P9-06,P11-02 | contracted browser workflows: conversation rename/delete (M-08), ordered source/template composer-ref discovery/attach while Evidence attach stays deferred (M-09/P11-04), administrator Model Provider Settings for credentials/profile visibility/global synthesis defaults with endpoint-specific CSRF/concurrency headers and immutable per-domain embeddings (A-01/A-02/A-13), plus safe domain/source operation-history UX (A-03/A-07/A-09/A-10); provider-settings parity trio approved — evidence `docs/_scratch/p9-07-contracted-browser-workflows-evidence.md`; plan `docs/plans/2026-07-28-010-feat-p9-07-contracted-browser-workflows-plan.md`; Playwright/visual matrix residual P12-07; provider packaging smoke residual P10-05 |
...
| P10-05 | DONE | P4-03,P5-04,P6-02,P7-03,P10-03 | production-real parser/provider pipeline: characterize and package Docling/Reducto, resolve bounded URL/assets and killable parser timeouts, bind immutable OpenAI/Bedrock embedding profiles for both index/query, prove long-block marker survival, and run a gated real-source→canonical blocks→LightRAG→mapped Evidence path; CI stays no-network and unsupported profiles remain fail-closed — plan `docs/plans/2026-07-28-012-feat-p10-05-provider-packaging-plan.md`; evidence `docs/_scratch/p10-05-provider-packaging-evidence.md` |
| P10-06 | DONE | P4-05,P9-03,P10-04 | deterministic governed PDF preview generation for supported non-PDF sources with private renderer/version/page-map/checksum metadata, atomic publication, safe range delivery, anchor fallback/remapping, and cleanup/recovery; original formats never reach the inline PDF renderer — plan `docs/plans/2026-07-28-017-feat-p10-06-governed-preview-generation-plan.md`; evidence `docs/_scratch/p10-06-governed-preview-evidence.md` |
...
| P11-03 | DONE | P11-02,P7 | private context assembly, turn fingerprint, replay/conflict and redaction — evidence `docs/_scratch/p11-03-assembly-fingerprint-replay-evidence.md`; browser discover unlock remains; P11-04 Evidence attach UX is DEFERRED |
| P11-04 | DEFERRED | P7-04,P9-02,P11-03 | product DEFER — Evidence suggest-and-confirm and Evidence attach-chip unlock not shipping until observed pain or grounding-quality gap (retrieval-first); validation `docs/_scratch/p11-04-evidence-reattachment-validation.md`; evidence `docs/_scratch/p11-04-evidence-reattachment-evidence.md`; plan `docs/plans/2026-07-28-001-feat-p11-04-evidence-attach-deferral-plan.md` |

The 2026-07-28 parser-to-retrieval pipeline review is dispositioned without restoring legacy architecture. P4-05 already closes normalized region persistence and authorized location/turn/viewer projection. P10-05 closed production parser packaging, bounded Reducto URL/asset handling, killable Docling execution, sealed OpenAI embedding bindings, marker-boundary proof, and gated staging smoke (operator live digests remain for production-supported labels). P10-06 closed the governed non-PDF preview pipeline (deterministic text/markdown/DOCX→PDF, generation-fenced publish, authorized delivery, anchor remap, derivative cleanup); PPT and renderer SBOM/live digests remain P12. P12-06 freezes the resulting parser/provider/renderer artifacts, while P12-07 consumes them through deployed browser, failure, capacity, privacy, and isolation scenarios. The read-only legacy implementation remains evidence for Docling item traversal, Reducto pointer resolution, asset materialization, and fixture shape only; its Redis locks, JSON domain manifests, provider metadata persistence, source-path heuristics, and competing `SourceChunk` authority remain rejected.

### P12 - Production release and recovery

Phase 1 scope decision (2026-07-28): `/database-visualize` must ship as a functional read-only Knowledge Domain graph rather than the previously approved unavailable placeholder. P12-07 begins by coordinating the higher-precedence governance, PRD, interaction, HTTP/DTO, architecture, frontend, fixture, and quality contract amendments; it then adds immutable per-domain graph-extraction profile binding, a private bounded LightRAG graph operation, a safe authorized product projection, an accessible Sigma/Graphology-based workbench adapted from `.references/code/context_engine/client`, and deployed demo/E2E proof. `scripts/dev.sh` becomes the lean supported full-stack demo entrypoint and must describe the Compose services, one-shot jobs, external integrations, and on-demand private domain runtimes without disclosing secrets or private endpoints. Direct browser-to-LightRAG access, raw vendor identifiers/properties, browser-selected runtimes, and entity/relation mutation remain prohibited. Because this changes schemas, contracts, dependencies, runtime images, security data flow, and backup/rebuild behavior after earlier P12 evidence, P12-07 must refresh affected P12-02..P12-06 artifacts before P12-08 may issue a go decision.
...
| P12-05 | IN_PROGRESS | P5-04,P7-06,P9,P12-02 | deployed-ingress incremental domain-RAG SSE, reconnect/replay, graceful shutdown and stream-drain proof through the real private LightRAG runtime — plan `docs/plans/2026-07-28-013-feat-p12-05-deployed-ingress-sse-drain-plan.md`; inventory/evidence `docs/_scratch/p12-05-deployed-ingress-{inventory,evidence}.md`; unit/config altitude landed (TLS overlay, API stop-new-turns `503 capacity_unavailable`, SSE inter-arrival helpers, proof scripts); live TLS AE1–AE4 operator digests still required for DONE |
| P12-06 | DONE | P0,P5-04,P10-04,P10-05,P10-06,P12-02 | immutable artifact manifest with pinned parser/provider/renderer and vendored LightRAG runtime images, MinIO/provider/runtime locks, schema and contract versions, SBOM and provenance — plan `docs/plans/2026-07-28-014-feat-p12-06-immutable-artifact-manifest-plan.md`; evidence `docs/_scratch/p12-06-immutable-artifact-evidence.md`; DONE at unit + PR-light altitude; residual: operator live Syft×built digests + cosign → P12-08 |
| P12-07 | IN_PROGRESS | P2-P5,P9-07,P10-04,P10-05,P10-06,P12-02,P12-03,P12-04,P12-05,P12-06 | coordinated Phase 1 graph contract amendment; immutable domain extraction-profile binding; private bounded LightRAG graph operation; safe generated graph API/DTO; accessible read-only `/database-visualize`; lean full-stack `scripts/dev.sh`; deterministic fixtures; full admin configure→Reducto upload→graph/index→chat→Evidence→PDF demo; browser E2E/CSRF/two-user cache; governed preview; runtime/provider failure; capacity/isolation; visual/a11y; and refresh of invalidated P12-02..P12-06 evidence — plan `docs/plans/2026-07-28-015-feat-p12-07-browser-e2e-capacity-plan.md`; inventory/evidence `docs/_scratch/p12-07-browser-e2e-capacity-{inventory,evidence}.md`; U1–U11 harness landed; residuals: approved PNG baselines, AT NVDA/VO, CI Playwright digest, live `@release` Reducto→graph→Evidence, P12-05 TLS graph path, P12-06 graph-aware SBOM refresh — **not DONE; B0 not complete** |
| P12-08 | NOT_STARTED | P12-03,P12-04,P12-05,P12-06,P12-07 | graph-aware, revision-matched production acceptance record, runbooks, recovery objectives and release decision; stale pre-graph evidence forces no-go — plan `docs/plans/2026-07-28-016-feat-p12-08-production-acceptance-plan.md` |
...
See `future/README.md`, `future/observability-layer.md`, `future/wiki-layer.md`, and the release-unassigned `future/reranking-layer.md`. Activating a future phase or unassigned capability requires a new approved contract and an updated build tracker on its named feature branch.
...
D0 created `DESIGN.md`, `frontend/AGENTS.md`, and parity/catalog rules. P9-01 closed the four unblocked starter targets and migration enforcement; P9-04 closed the Settings Domain accordion at Vitest altitude. **P9-06** delivered the Option A full Phase 1 workstation HTML gallery. **P9-07** closed contracted browser workflows (rename/delete, source/template composer refs, Model Provider Settings, operation history) and the `provider-settings` parity target. Script-free synthetic HTML may prove static appearance only; React owns semantics/focus/accessibility. P12 browser acceptance for Settings routes and the route visual matrix must use the production Next build, same-origin BFF and FastAPI with server-produced DTOs, without intercepted or mocked product responses, and must reference catalog `targetId`s.
