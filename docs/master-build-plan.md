...
| P9-07 | NOT_STARTED | P9-02,P9-03,P9-04,P9-06,P11-02 | contracted browser workflows: conversation rename/delete (M-08), ordered source/template composer-ref discovery/attach while Evidence attach stays deferred (M-09/P11-04), administrator Model Provider Settings for credentials/profile visibility/global synthesis defaults with endpoint-specific CSRF/concurrency headers and immutable per-domain embeddings (A-01/A-02/A-13), plus safe domain/source operation-history UX (A-03/A-07/A-09/A-10); approve a provider-settings parity trio using `docs/_scratch/provider-settings-imagined.html` as non-normative guidance — plan `docs/plans/2026-07-28-010-feat-p9-07-contracted-browser-workflows-plan.md` |
...
D0 created `DESIGN.md`, `frontend/AGENTS.md`, and parity/catalog rules. P9-01 closed the four unblocked starter targets and migration enforcement; P9-04 closed the Settings Domain accordion at Vitest altitude. **P9-06** expands to Option A: full Phase 1 workstation HTML gallery (primitives, shared compositions, feature composites including chat). Script-free synthetic HTML may prove static appearance only; React owns semantics/focus/accessibility. P12 browser acceptance for `/settings?section=domains` and the route visual matrix must use the production Next build, same-origin BFF and FastAPI with server-produced DTOs, without intercepted or mocked product responses, and must reference catalog `targetId`s.
