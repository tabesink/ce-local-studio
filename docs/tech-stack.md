...
- `scripts/verify.sh` and `.github/workflows/verify.yml` are the pinned root verification loop (P12-02). Default verify covers phase-scope, locks, backend lint/pytest (including P8 privacy scans; postgresql-marked tests skip without opt-in), six-artifact OpenAPI/SSE/DTO/TypeScript live+adversarial snapshot gates, frontend lock/typecheck/test/build, backend Docker image build, and Compose config. A required parallel `verify-postgresql` job runs `pytest -m postgresql` against disposable PostgreSQL 16. Live Compose smoke stays evidence-owned (P10); Playwright/browser E2E and deployed-ingress remain P12-07/P12-05 — this is not B0 or release evidence.
- `scripts/generate_openapi.py` / `scripts/generate_json_schemas.py` and the six committed artifacts under `app/contracts/` plus `app/client/src/lib/api/generated/` regenerate and byte-compare under the root gate; production and generation share route registration. The registered/catalog route delta and broader handwritten response-DTO adoption remain recorded in `docs/_scratch/p0-06-generated-contract-inventory.md` / DRIFT-01.
