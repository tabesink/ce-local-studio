...
Workflow: human edits `tests/parity/fixtures/<targetId>.html` → agent mirrors into the owning React module → Vitest/RTL parity passes → catalog state may become `FACTORY_READY`. HTML never authorizes product behavior. Do not invent a shared Accordion primitive or storageSummary UI. Do not add P11-04 Evidence attach/suggest gallery targets while that work is DEFERRED. Graph gallery follows the approved read-only graph contract; U10 replaces the former unavailable surface with enabled workbench targets that keep searchable list/detail parity with the canvas.
