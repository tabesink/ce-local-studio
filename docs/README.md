...
This package defines the target contract and evidence-led brownfield refactor needed to align the lifted Context Engine application.

## Package map

- `master-build-plan.md` - phased implementation tracker, dependencies, gates, and status.
- `brownfield-refactor-register.md` - complete DRIFT-01 through DRIFT-33 disposition, reusable-foundation reverify gates, and current red baseline.
- `phase-scope-manifest.md` - machine-read Phase 1 routes, ref kinds, retained safety baseline, removed surfaces, tombstones, child ceilings, and governed-file classifications.
...
- `architecture/data-and-lifecycle.md` - authoritative ownership, privacy sinks, outbound ports, and ingestion/retrieval/chat/redaction state-machine conventions.
...
- `architecture/legacy-persistence-retirement.md` - static legacy-closure hypothesis and populated-database compatibility stop condition.
...
- root `DESIGN.md` - subordinate CE visual constitution.
- `frontend/AGENTS.md` - frontend coding-agent read order, layering, safety, and factory boundaries.
...
Review basis: the current root `app/` and `scripts/` trees, the dated review in `docs/_scratch/code-docs-drift-review.md`, and the phase-trimmed authority source in `.references/ce-local-studio-no-wiki-observability/docs/`. This workspace has no usable Git provenance, so no completion claim may be inferred from branch or commit history.
...
The lifted application includes client, server, vendored LightRAG, container/environment files, and a root development script, but its supplied launch paths and verification baseline are red. This package therefore distinguishes target behavior from current-tree evidence and schedules inventory, disposition, remediation, and proof before any existing seam receives completion credit.
